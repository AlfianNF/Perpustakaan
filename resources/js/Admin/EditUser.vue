          ...
          <div v-if="user">
            <form @submit.prevent="updateUser">
              <div class="mb-4">
                ...
                <input v-model="user.name" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="name" type="text" placeholder="Nama" />
              </div>
              <div class="mb-4">
              ...
              <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="no_induk">No Induk</label>
                <input v-model="user.no_induk" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="no_induk" type="text" placeholder="No Induk" readonly />
              </div>
              <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="is_admin">Is Admin</label>
                <select v-model="user.is_admin" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="is_admin">
              ...
              <div class="mb-4">
                ...
                <input type="file" @change="onFileChange" accept="image/*" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="image" />
                <div v-if="previewImage" class="mt-2">
                  <img :src="previewImage" alt="Preview Gambar" class="w-32 h-32 object-cover rounded-full" />
                </div>
                <div v-else-if="user.image" class="mt-2">
                  <img :src="getImageUrl(user.image)" alt="Gambar Saat Ini" class="w-32 h-32 object-cover rounded-full" />
                </div>
              </div>
    ...
    methods: {
      async fetchUser() {
        ...
        try {
          ...
          const response = await axios.get(`${this.baseURL}api/user/${this.$route.params.id}/show`, {
      ...
      async updateUser() {
        try {
          ...
          await axios.post(`${this.baseURL}api/user/${this.$route.params.id}/update`, formData, {
            headers: {
